...
def plot_extreme_slices(unshimmed, shimmed_masked, mask, softmask, path_output=None, mask_seg=None):
    ...
    print(f"Slice with max mean: {idx_max}, Slice with min mean: {idx_min}")
    ...
    for i, idx in enumerate(slices_to_show):
        ...
        ax1.set_title(f"{titles[i]}\nAvant simulation de shimming", fontsize=14, color="#0D1B2A")
        if mask_seg is not None:
            seg_slice = np.rot90(seg_data[:, :, idx])
            ax1.contour(seg_slice, levels=[0.5], colors='#B08D57', linewidths=1, zorder=10)
        ax1.axis('off')
        ...
        im2 = ax2.imshow(np.rot90(masked_shimmed), vmin=vmin, vmax=vmax,
                         cmap=royal_white_bordeaux, aspect='equal', interpolation='nearest')
        ax2.set_title("Tranche correspondante\nAprès simulation de shimming", fontsize=14, color="#0D1B2A")
        if mask_seg is not None:
            ax2.contour(seg_slice, levels=[0.5], colors='#B08D57', linewidths=1, zorder=10)
        ax2.axis('off')
    ...
    legend_element = Line2D([0], [0], color='#B08D57', lw=2, label='Contour de\nla moelle\népinière')
