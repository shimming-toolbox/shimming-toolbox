...
echo Create Shimming Toolbox directory
...
REM Download mamba
powershell -Command "Invoke-WebRequest %CONDA_INSTALLER_URL% -OutFile %UNIQUE_TMP_INSTALLER% || goto error

REM Install mamba
start /wait %UNIQUE_TMP_INSTALLER% /RegisterPython=0 /S /D=%ST_DIR%\%PYTHON_DIR% || goto error
...
echo cp "%ST_SOURCE_FILES%\config\dcm2bids.json" "%ST_DIR%\dcm2bids.json"
