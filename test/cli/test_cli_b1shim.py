...
path_b1_nii_axial = os.path.join(path_b1_folder, 'sub-tb1tfl_run-01_TB1map_uncombined.nii.gz')
path_b1_nii_coronal = os.path.join(path_b1_folder, 'sub-tb1tfl_run-02_TB1map_uncombined.nii.gz')
path_b1_nii_sagittal = os.path.join(path_b1_folder, 'sub-tb1tfl_run-03_TB1map_uncombined.nii.gz')
...
def test_b1shim_cli():
    ...
    # Run the CLI
    result = CliRunner().invoke(b1shim_cli, ['--b1map', path_b1_nii_axial], catch_exceptions=True)
...
def test_b1shim_cli_args():
    ...
    with tempfile.TemporaryDirectory(prefix='st_' + pathlib.Path(__file__).stem) as tmp:
        ...
        # Run the CLI
        result = CliRunner().invoke(b1shim_cli, ['--b1map', path_b1_nii_axial, '--mask', fname_mask, '--algo', '2',
...
def test_b1shim_cli_coronal():
    ...
    with tempfile.TemporaryDirectory(prefix='st_' + pathlib.Path(__file__).stem) as tmp:
        ...
        # Run the CLI
        result = CliRunner().invoke(b1shim_cli, ['--b1map', path_b1_nii_coronal, '--mask', fname_mask, '--algo', '2',
                                                 '--target', '20', '--vop', path_sar_file, '--sar_factor', '1.2',
                                                 '--output', path_output], catch_exceptions=True)
        assert len(os.listdir(path_output)) != 0
        assert result.exit_code == 0
...
def test_b1shim_cli_sagittal():
    ...
    with tempfile.TemporaryDirectory(prefix='st_' + pathlib.Path(__file__).stem) as tmp:
        ...
        # Run the CLI
        result = CliRunner().invoke(b1shim_cli, ['--b1map', path_b1_nii_sagittal, '--mask', fname_mask, '--algo', '2',
                                                 '--target', '20', '--vop', path_sar_file, '--sar_factor', '1.2',
                                                 '--output', path_output], catch_exceptions=True)
        assert len(os.listdir(path_output)) != 0
        assert result.exit_code == 0
