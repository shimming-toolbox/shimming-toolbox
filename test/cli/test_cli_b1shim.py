...
def test_b1shim_cli():
    ...
    # Run the CLI
    runner = CliRunner()
    result = runner.invoke(b1shim_cli, ['--b1map', fname_b1_map], catch_exceptions=True)
...
def test_b1shim_cli_args():
    ...
    with tempfile.TemporaryDirectory(prefix='st_' + pathlib.Path(__file__).stem) as tmp:
        ...
        # Run the CLI
        runner = CliRunner()
        result = runner.invoke(b1shim_cli, ['--b1map', fname_b1_map, '--cp', fname_cp_json, '--algo', 2, '--target',
