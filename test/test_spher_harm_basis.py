...
class TestSiemensBasis:
    def test_siemens_basis(self, x, y, z):
        basis = siemens_basis(x, y, z, orders=(1, 2))
...
class TestGEBasis:
    ...
    def test_ge_basi_order1(self, x, y, z):
    ...
    def test_ge_basi_order2(self, x, y, z):
...
class TestPhilipsBasis:
    def test_philips_basis(self, x, y, z):
        # Off center z axis
        basis = philips_basis(x, y, z, orders=(1, 2))
        # Test for shape
        assert (np.all(basis.shape == (x.shape[0], x.shape[1], x.shape[2], 8)))
        # X, Y, Z, Z2, ZX, ZY, X2 - Y2, XY
...
class TestGetFlipMatrix:
    ...
    def test_flip_ge(self):
        out = get_flip_matrix('LPI', manufacturer='GE')
        assert np.all(out == [-1, -1, -1, 1, 1, 1, 1, 1])

    def test_flip_philips(self):
        out = get_flip_matrix('RPI', manufacturer='PHILIPS')
        assert np.all(out == [1, -1, -1, 1, -1, 1, 1, -1])
