...
def multi_dynamic_no_cli(fname_fmap_avg, fname_fmap, fname_anat, fname_mask_anat, method, opt_criteria, slices, slice_factor, coils,
    ...
    # Load the fieldmap
    nii_fmap_orig = nib.load(fname_fmap_avg)
    ...
    metric_unshimmed_mean = np.zeros(nii_fmap_rt.shape[-1])
    metric_shimmed_std = np.zeros(nii_fmap_rt.shape[-1])
    ...
    return metric_shimmed_mean.mean(), metric_unshimmed_mean.mean(), metric_shimmed_std.mean(), metric_unshimmed_std.mean(), metric_shimmed_absmean.mean(), metric_unshimmed_absmean.mean()
