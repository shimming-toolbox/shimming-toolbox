...
class NumericalModel:
    ...
    volume = {"T2_star": None, "proton_density": None}
    ...
    def __customize_shepp_logan__(self, volume, class1, class2, class3):
    ...
    def simulate_measurement(self, FA, TE, SNR=None):
        ...
        if len(volDims) == 2:
            self.measurement = np.zeros(
                (volDims[0], volDims[1], 1, numTE), dtype="complex_"
            )
        elif len(volDims) == 3:
            self.measurement = np.zeros(
                (volDims[0], volDims[1], volDims[2], numTE), dtype="complex_"
            )
    ...
    def add_noise(volume, SNR):
        ...
        noisyReal = (
            np.real(volume)
            + np.random.randn(volDims[0], volDims[1], volDims[2], volDims[3]) * noiseSTD
        )
        noisyImaginary = (
            np.imag(volume)
            + np.random.randn(volDims[0], volDims[1], volDims[2], volDims[3]) * noiseSTD
    ...
    def save(self, dataType, fileName, saveFormat=None):
        ...
        if fileName[-4:] == ".nii":
            if saveFormat != "nifti":
                print(
                    "File extension and saveFormat do not match - saving to NIfTI format"
                )
                saveFormat = "nifti"
            ...
            if saveFormat != "mat":
                print(
                    "File extension and saveFormat do not match - saving to MAT format"
                )
                saveFormat = "mat"
            fileName = fileName[0:-4]
    ...
    def writeJson(self, fileName):
        pulseSeqProperties = {"EchoTime": self.TE, "FlipAngle": self.FA}
